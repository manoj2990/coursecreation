...
const AssessmentStep = ({ data, onUpdate, onNext, onPrev }: AssessmentStepProps) => {
  ...
  const [currentAssessment, setCurrentAssessment] = useState<Assessment>({ title: "", questions: [] });
  ...
  const saveAssessment = () => {
    ...
    setAssessments(prev => {
      const existing = prev.find(sa => sa.subtopicId === subtopicId);
      if (existing) {
        return prev.map(sa => 
          sa.subtopicId === subtopicId 
            ? { ...sa, assessments: [...sa.assessments, currentAssessment] }
            : sa
        );
      } else {
        return [...prev, { subtopicId, assessments: [currentAssessment] }];
      }
    });

    setCurrentAssessment({ title: "", questions: [] });
  ...
  const publishAssessments = async () => {
    try {
      const payload = {
        courseId: data.id,
        topicId: selectedTopicIndex !== null ? topics[selectedTopicIndex].id : "",
        subtopics: assessments.map(sa => ({
          subtopicId: sa.subtopicId,
          assessments: sa.assessments
        }))
      };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={publishAssessments} disabled={assessments.length === 0}>
          Continue to Publish
